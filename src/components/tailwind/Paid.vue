          ...
          <li class="p-4 sm:p-6">
            <div class="flex items-center sm:items-start">
              ...
              <div class="ml-6 flex-1 text-sm">
                ...
                <p>
                  <span class="font-medium text-sm text-gray-500">{{ order.address }}<span class="ml-2">{{order.consignee}}</span><span class="ml-1">{{order.phone}}</span></span>
                </p>
                <p>
...
const showOrders = async () => {
  const response = await axios.post('http://124.221.7.201:8081/order/getOrdersByUserId', {
    userId: userid.value,
  })
  ...
  if (response.data.code === 200) {
    ...
    // 请求所有商品的数据
    const productRequests = orderData.map(order => axios.get('http://124.221.7.201:8081/product/selectById', {
      params: {
...
const deleteOrder = async (id) => {
  try {
    const response = await axios.delete('http://124.221.7.201:8081/order/deleteOrder', {
      params: {
        id: id,
      }
    })
